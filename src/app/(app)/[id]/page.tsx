    ...
    <div className="mx-auto w-full lg:w-[1280px] items-center grid gap-12 py-6 grid-cols-1 lg:grid-cols-2">
            ...
            <CarouselContent>
              { loading ? (
                Array.from({length: 3}).map((_, index) => (
                  <CarouselItem key={index} className="basis-3/2  lg:basis-1/4">
      ...
      <div className="flex flex-col w-full justify-between  h-full">
        ...
        <div className='flex flex-col gap-1'>
          <h2 className="text-xl font-bold">{immobile?.name && `${immobile.name} |`} {immobile?.address}</h2>
          <span className="text-sm text-muted-foreground leading-snug">A Partir de</span>
          <span className="mb-6 flex items-end justify-between text-4xl font-bold">{immobile?.price}<strong className='text-muted-foreground text-xs'>C. {immobile?.code}</strong> </span>
        </div>
